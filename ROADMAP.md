...
> 更新日期：2026-05-17
> 目前線上版本：**v3.0**（音效音樂 + 列印單 + 配對工具 + 徽章 + 匯出）
...
#### 16. 📱 PWA 化（可加到桌面、離線玩）
- 📊 🌟 中 ｜ ⏱️ 0.5 天 ｜ 🎯 行動體驗升級
- 🔧 Next.js + `next-pwa` 或手寫 service worker
- **內容**：
  - 加 `manifest.json` 配色、icon、name
  - Service Worker 快取所有靜態資源
  - 手機桌面 icon、splash screen
  - 離線可玩（資料都在前端）
- **注意**：依 skill `pwa-cache-bust` 處理版本快取問題
