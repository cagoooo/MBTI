...
> 更新日期：2026-05-18
> 目前線上版本：**v3.22.5**（服務組完整化 + Piano Warm BGM + 列印 bug 修補 + 5 大未來方向擴充規劃）
...
### ✅ v3.22.3 — 服務組專屬 BGM「Piano Warm」（2026-05-18）

> Agent P3 建議的「服務組應該有自己的聲音識別」, 跑 Chrome MCP + Pixabay 工作流 (skill `pixabay-audio-asset-pipeline`) 抓專屬曲。

| 項目 | 細節 |
|---|---|
| **曲名** | Piano Warm by Music_For_Videos |
| **時長 / 大小** | 1:24 / 2.6 MB (介於 home 4.7MB / game 2.9MB 之間) |
| **License** | Pixabay Content License (CC0 等價) |
| **Pixabay 標籤** | "Background **Charity**" — 完美對應「公民關懷」氣質 |
| **工作流** | navigate → get_page_text → find → regex 抓 cdn URL → PowerShell 下載 → sound.ts |
| **整輪時間** | < 5 分鐘 (skill 模板加速) |

8 條 BGM track 完整陣容: home / game / result / sport / art / study / friend / **service** 🆕

### ✅ v3.22.4 — 列印結果單空白修補（2026-05-18）

> 使用者實機列印 iPhone Safari 看到 A4 預覽完全空白 — v3.17 設計系統 rebuild 時的隱形雷 2 個月後浮現。

| 項目 | 細節 |
|---|---|
| **症狀** | 點「📄 列印我的結果單」→ 列印對話框 A4 完全空白 (iOS Safari 最明顯, 所有瀏覽器都中) |
| **根因** | `result/[type]/page.tsx:70` 包進 `<div className="container-paper screen-only">`, PrintSheet 在 wrapper 內。`@media print` 把 `screen-only` 設 `display:none`, **整個 subtree 從 layout tree 拔掉**, 子層 PrintSheet 的 `display:block !important` 救不了 |
| **修法** | PrintSheet 移到 wrapper **外面** (用 Fragment 包):<br>`<><PrintSheet/><div screen-only>...</div></>` |
| **同類雷掃描** | worksheet 用「兄弟元素 print-hide」模式 OK / EmergencyCard 用 body.printing-card class OK / 只有 result/[type] 中雷 |
| **固化進 skill** | `pdf-export-print-best-practice` 加新章節「print-sheet 子層被 screen-only 父層 hide」+ 4 種 Pattern 對照表 + 全專案 grep 指令 |

### ✅ v3.22.5 — 列印單左側橘線修補（2026-05-18）

> v3.22.4 修好「空白」後, 列印單左側出現一條橘色直線。

| 項目 | 細節 |
|---|---|
| **症狀** | 列印結果單左側出現一條橘色直線 (不該有) |
| **根因** | v3.17 紙感設計加了 `body::before` 紅 margin 線 + `body::after` 三圓點裝飾, `position: fixed`。`@media (max-width: 720px)` 有 hide, **但通用 `@media print` 忘了 hide**, iOS Safari 把 fixed 元素印進 PDF |
| **修法** | globals.css 第 2126 行 `@media print` 區塊加:<br>`body::before, body::after { display: none !important; }` |
| **為什麼 EmergencyCard 沒事** | 第 780 行 `@media print` 有處理但限 `body.printing-card` class, 只 EmergencyCard 有加; 通用 PrintSheet 走另一條 print 規則 |
...
## 🆕 v3.22 後的下一波建議（2026-05-18 補充）

> 經過 v3.19~v3.22 大量功能 + 內容擴充後 (PWA / RWD / Story Pack 4 個 / 6 週教案 / Print 修補), **整個產品已經有完整生態系**。
> 下一波重點不再是「補功能」, 而是「**內容深度 + 影響力 + 跨平台 + 永續經營**」。

字母前綴接續用到 AK, 這波從 **AL** 開始。

---

### 🎓 AL. 深化「教育研究 × 影響力」(學術 + 推廣)

> 累積這麼多教學現場資料 + 完整教案, 是時候轉化成可被學術界 / 政府引用的成果。

#### AL1. 📝 投稿《國民教育電子報》或《親子天下》使用心得
- 📊 🔥 高 ｜ ⏱️ 3 天 ｜ 🎯 教師同行看見 + 個人專業檔案
- 主題建議: 「用 Claude Code 半年從零打造全國推廣的 MBTI 校園 RPG」
- 結構: 教學動機 + 4 個 Pack 設計取捨 + 學生反饋 + 系統架構 + 持續迭代心得
- 投稿管道: 親子天下教師選讀 / 國教院電子報 / 桃園市教師期刊 / 數位學習電子報
- **產物**: 1 篇 1500-2000 字 + 1 份 fork-friendly README + 5-8 張示意圖

#### AL2. 🧪 跟教育心理系合作做實證研究
- 📊 🌟 中 ｜ ⏱️ 1 學期觀察 + 寫論文 ｜ 🎯 SSCI 期刊發表
- 合作對象: 政大教育系 / 師大心理系 / 雲科大資管所
- 設計: 實驗組 (完整跑 6 週) vs 對照組 (沒跑), 期末用 Sociogram 測人際網絡變化
- 同時量 SEL 能力 (CASEL 量表) + 多元接納度
- **產物**: 1 篇學術論文 + 政府推廣材料

#### AL3. 🎤 製作 5 集教學使用案例 podcast
- 📊 🌟 中 ｜ ⏱️ 5 集 × 0.5 天 ｜ 🎯 教師同行 + 家長
- 每集主題 (15 分鐘):
  - EP1 — 為什麼國小要做 MBTI? 我的反思
  - EP2 — SEL 9 情境怎麼設計? 兒福聯盟資料 + CASEL 框架
  - EP3 — 班級活動跑流程實況 (錄真實一節課)
  - EP4 — 家長日如何用 AI 班級洞察報告
  - EP5 — 跨校老師反饋 + 改進心得
- 平台: SoundOn / Apple Podcast / Spotify
- **產物**: 5 集音檔 + 5 篇文字稿

#### AL4. 🌐 教學白皮書中英雙語版
- 📊 🌙 低 ｜ ⏱️ 5 天 ｜ 🎯 國際華僑學校 + 跨國推廣
- 主體: 已有的 6 週教案 + 系統架構 + 教學心得
- 結構: 中英對照 PDF, 30-50 頁
- **產物**: 投教育部國際處 / 海外華僑學校網絡

#### AL5. 🎓 「校園奇遇記 × 教師工作坊」線上 / 線下系列
- 📊 🔥 高 ｜ ⏱️ 籌備 2 週 + 每月 1 場 ｜ 🎯 教師社群形成
- 主題: 半天 / 全天工作坊, 教 30-50 位老師如何在班上跑活動
- 結業頒「校園 MBTI 引導師」電子證書 (PDF 自製徽章)
- 結業老師標記在 V1 全國地圖
- 跟教育部「終身學習時數」掛勾, 可申請認證

---

### 🌐 AM. 跨年齡 / 跨平台擴展

> 目前是「國小版」+ 「Web」, 可以擴出去服務更多年齡層 + 更多管道。

#### AM1. 🎒 國中初探版 Story Pack
- 📊 🔥 高 ｜ ⏱️ 5-7 天 ｜ 🎯 銜接國中, 擴大年齡層
- 沿用 6 章結構, 場景改成: 跨年級社團 / 學測壓力 / 異性朋友 / 第二外語社群
- MBTI 計分軸一致, 但情境深度提升 (例: 親密關係、性別認同探索初階)
- 跟服務組 / 數位素養 / 家庭篇可以共用 (情境換成國中)
- **配套**: 國中老師導讀手冊 + 7-9 年級對應建議
- **路徑**: `/junior` 獨立路由 (跟現有國小 `/game` 並存)

#### AM2. 👨‍👩‍👧 家長版 MBTI Story Pack
- 📊 🌟 中 ｜ ⏱️ 5 天 ｜ 🎯 親師活動 / 親子討論
- 重新包裝成成人版 (職場、家庭、社交、伴侶)
- 學生帶回家 QR Code, 全家一起玩
- AI 自動生成「親子討論題目」 (「你是 ENFP 媽媽, 孩子是 INTJ, 可以這樣談...」)
- **路徑**: `/parent` 獨立路由 + 「掃此 QR 全家一起玩」分享卡

#### AM3. 🤖 LINE Bot 互動版 (學生最熟悉平台)
- 📊 🌟 中 ｜ ⏱️ 3 天 ｜ 🎯 學生最熟悉, 容易擴散
- 用 LINE Messaging API + Firebase Cloud Functions (skill `line-messaging-firebase`)
- 純文字 + 選按鈕版的簡化 MBTI (10 個情境)
- 結果直接傳家長 LINE 群
- LINE LIFF 連回完整網頁版深入查看
- 老師可建立 LINE 群讓 bot 自動統計

#### AM4. 🎴 桌遊 / 卡牌實體版
- 📊 🌙 低 ｜ ⏱️ 設計 1 月 + 印製 1 月 ｜ 🎯 沒網路的學校 / 老人安養院 / 偏鄉
- 用現有 12 NPC 卡牌 + SEL 9 情境 + 數位素養 6 情境設計實體版
- 每張卡 A6 大小 (與 EmergencyCard 同尺寸), 雙面印
- 配 1 本 30 頁老師導引手冊 + 1 個遊戲說明影片
- **採購對象**: 國教輔導團 / 偏鄉教育基金會
- 收費 NT$ 500-800 / 套, 部分款項回饋持續開發

#### AM5. 📱 PWA 進階 — 推播通知 + 真離線
- 📊 🌟 中 ｜ ⏱️ 2 天 ｜ 🎯 學校沒網路也能用 + 老師活動提醒
- 已有: Service Worker + offline.html + manifest shortcuts
- 待補:
  - **推播通知** (Firebase Cloud Messaging): 老師「下次活動建議」提醒 / 學生 SEL 7 天打卡提醒
  - **真離線可玩**: pre-cache 主故事 30 個場景 + 4 個 Pack 的 JSON, 完全斷網也能跑完
  - **背景同步**: 學生離線玩完, 上線時自動上傳結果

---

### ✨ AN. 學生個人化 + AI 學習助手深化

> Gemini 已串好, 可做更深的個人化體驗給學生。

#### AN1. 💬 「跟你的 MBTI 同型 NPC 聊天」(真實聊天版)
- 📊 🔥 高 ｜ ⏱️ 3 天 ｜ 🎯 學生超愛 + 角色扮演沉浸感
- 結果頁加「💬 跟同型同學 X 聊聊」chat 介面
- AI 扮演該型 NPC (從 12 個 NPC 中對應), 用該角色設定 + MBTI 個性
- 例: ENFP 結果 → 跟「小芸」聊天, AI 用小芸的活潑口吻 + 客家阿婆故事
- **嚴格守門**: 話題白名單 (學校 / 興趣 / 家庭 / 情緒) + 不討論時事 / 政治 / 暴力 / 性
- 每天 chat 上限 10 句 + 老師後台可關
- **隱私**: 不存對話紀錄到後端, 純 sessionStorage

#### AN2. 🎯 個人化「下週練習一件事」打卡系統
- 📊 🌟 中 ｜ ⏱️ 2 天 ｜ 🎯 SEL 從一次性活動變持續習慣
- 完成 SEL 後選一個工具箱項目當「下週目標」
- 每天打開網站跳「今天有試試 OO 嗎?」7 天勾選
- 連續勾 7 天 → 解鎖「情緒練習者」徽章 + 慶祝動畫
- 整合進 `/me` 顯示打卡熱度圖 (像 GitHub contribution graph)
- **資料**: 純 localStorage, 跨次保存

#### AN3. 📔 「給未來自己的一封信」時光膠囊
- 📊 🌟 中 ｜ ⏱️ 1.5 天 ｜ 🎯 SEL 反思深化的浪漫設計
- SEL 結果頁加「寫一封信給 3 個月後的自己」
- 填完存 localStorage + 顯示預定打開日期
- 3 個月後重訪 `/me` 跳出「💌 3 個月前你寫給自己的信來了!」
- 「我猜對了嗎?」對比現在的狀態 (MBTI / SEL 風格變化)
- 純前端, 不需後端

#### AN4. 🧠 AI 學習教練 (每週評估)
- 📊 🌟 中 ｜ ⏱️ 2 天 ｜ 🎯 給學生持續成長伴侶
- `/me` 學習歷程冊加「📊 AI 教練週報」
- Gemini 結合所有歷史 (MBTI / SEL / 猜朋友) 產出:
  - 「這週你的 MBTI 軌跡」(若有變化, 解讀為什麼)
  - 「下週可以練習的一件事」(根據最弱軸)
  - 「適合你的小書 / 活動推薦」(根據型 + 風格)
- 每週只能跑 1 次, 省 Gemini quota
...
> v3.21 跨班級總覽是起點, 可深化成完整老師工作流。

#### AO1. 🍴 「全國使用學校地圖」+ fork 展示頁
- 📊 🔥 高 ｜ ⏱️ 1.5 天 ｜ 🎯 推廣 ROI 爆表 + 教師社群形成
- 新頁 `/community/schools` 列出所有 fork 過此網站的學校 + 連結
- 老師 fork 後填表回報「OO 國小 + 網址 + 心得」
- 用 Firebase RTDB `forks/` collection 收集
- 顯示台灣地圖 (Mapbox or Leaflet 免費版) 標記 + 學校數計數器
- dashboard 顯示「全國有 X 校在用此網站」鼓勵感

#### AO2. 👥 同校老師共筆 + 學年導師接班
- 📊 🔥 高 ｜ ⏱️ 4 天 ｜ 🎯 學校教研社群形成
- Firebase RTDB 加 `schools/{domain}/teachers/{uid}` collection
- 同 Google email domain (smes.tyc.edu.tw) 自動同校
- 共筆每個班的「個性備註」/「適合分組」/「需注意觀察的學生」
- **學年接班**: 5 年級導師交給 6 年級導師時, 前老師留的備註自動可見
- 隱私: 學生只顯示 displayName 別名

#### AO3. 📚 老師備課議題討論社群 (Discord / LINE)
- 📊 🌟 中 ｜ ⏱️ 籌備 1 週 + 日常維運 ｜ 🎯 形成教師同行社群
- 建立 Discord 伺服器 (或 LINE 社群):
  - #公告 (新版本 / 教案更新)
  - #教學經驗分享 (老師講班上實況)
  - #改編 fork (老師改造的版本互相分享)
  - #問題討論 (使用上的疑問)
  - #SEL-輔導課 / #數位素養 / #家庭篇 (各 Pack 專版)
- 你 (阿凱老師) 每週進來看 30 分鐘 + 回問題

#### AO4. 📊 老師「我的教學儀表板」(個人專業檔案匯出)
- 📊 🌟 中 ｜ ⏱️ 2 天 ｜ 🎯 教評會神器 + 教師甄選
- dashboard 加「📥 匯出我的教學專業檔案 PDF」
- 內容: 我用了 N 次活動 / 輔導 M 位學生 / 跨 K 個班級 / 學期成果摘要
- 含學校 logo / 教師頁連結 / 統計圖表
- 適合放教評會 / 教師專業發展檔案 / 學年初家長會 ice-breaker
...
- Gemini 結合班級資料 + 教學經驗回答
- 對話歷史存 sessionStorage
- 包括: 教案撰寫協助 / 學生個案討論 / 家長溝通模板 / 反思題建議

---

### ♿ AP. 無障礙完整化 + 跨年齡 + 跨能力

> 學校採購硬需求, 也是社會責任 — 讓所有學生不論能力都能玩。

#### AP1. ♿ axe-core 完整稽核 + WCAG 2.1 AA 認證
- 📊 🔥 高 ｜ ⏱️ 2 天 ｜ 🎯 學校採購硬需求 + 教育部規範
- 跑 axe-core 全頁掃描 (16 頁 + 4 Pack + dashboard)
- 補: 所有按鈕 aria-label / 對話氣泡 role="dialog" / 鍵盤 nav / 顏色對比 ≥4.5:1
- screen reader 試讀 (NVDA Windows / VoiceOver Mac / TalkBack Android)
- 取得 WCAG 2.1 AA 自評認證 + 證明文件

#### AP2. ⌨️ 完整鍵盤導航 + 快捷鍵系統
- 📊 🌟 中 ｜ ⏱️ 1.5 天 ｜ 🎯 行動不便 + 老師快速操作
- 場景選項 `1` / `2` / `3` / `4` 鍵直接選
- `/teacher/dashboard` 快捷鍵: `G` 跳遊戲 / `H` 跳歷史 / `D` 新房間 / `S` 設定
- Tab order + focus ring 強化 (現在有, 但 visual 需更明顯)
- ARIA live region 朗讀 followUp + flag warning

#### AP3. 🎨 高對比 / 色盲友善 / 大字模式
- 📊 🌟 中 ｜ ⏱️ 1.5 天 ｜ 🎯 視障 + 色弱 + 高齡輔助
- SettingsPanel 加色彩模式 toggle (3 種):
  - 預設 (校園手帳紙感)
  - 高對比 (黑白 + 加粗框線, 對比 ≥7:1)
  - 色盲友善 (Deuteranopia 主, 避免紅綠混用)
- 字級已有 4 段, 可加第 5 段「特大」(2x)
- 結合 `prefers-contrast` / `prefers-color-scheme` media query

#### AP4. 🔊 完整聲控操作 (Web Speech Recognition)
- 📊 🌙 低 ｜ ⏱️ 3 天 ｜ 🎯 視障 / 行動不便 / 識字弱
- 用瀏覽器原生 SpeechRecognition API
- 聲控指令: 「選一」「選二」「下一段」「再唸一次」「停止」
- 配合 TTS 達成完全免手操作
- 開啟 / 關閉在 SettingsPanel

#### AP5. 🌏 多語版本 (繁中 / 簡中 / 英文 / 越南)
- 📊 🌙 低 ｜ ⏱️ 7 天 (含翻譯) ｜ 🎯 國際推廣 + 新住民學生友善
- next-intl + JSON locale 檔
- 4 版本:
  - zh-TW (預設, 現有)
  - zh-CN (簡體, 海外華僑 / 跨海合作)
  - en-US (國際學校)
  - **vi-VN** (越南, 對應台灣新住民第二代如阿哲)
- SEL / 家庭篇 內容國際通用 (CASEL 是美國框架本來就有英文版)
- MBTI 主故事文化保留台灣特色, 用 footnote 解釋校園情境

#### AP6. 🎒 特教專用模式 (情緒卡 + 圖卡)
- 📊 🌙 低 ｜ ⏱️ 4 天 ｜ 🎯 自閉 / 學障 / 發展遲緩學生友善
- 新模式 `/special` — 用視覺輔助溝通 (AAC) 風格
- 把 9 個 SEL 情境改成「情緒圖卡」(每張一張臉 + 一個情境)
- 選項用大圖卡 + 簡單文字 + TTS
- 配色低刺激 (柔和米色 / 灰藍)
- **配套**: 特教老師導引手冊 + ABA 行為分析建議

---

## 🥇 阿凱老師建議下一波最優先（v3.22 後）

### 🥇 立刻做（一週內、可獨立完成、ROI 爆表）

| 優先 | 項目 | 工時 | 為什麼這麼急 |
|---|---|---|---|
| 🥇 | **AP1** axe-core a11y 完整稽核 | 2 天 | 學校採購硬需求, 從 v3.13 拖到現在 |
| 🥇 | **AL1** 投稿教育月刊使用心得 | 3 天 | 1 篇文章傳遍全國老師, 個人專業檔案 |
| 🥇 | **AO1** fork 全國地圖 + 展示頁 | 1.5 天 | 推廣 ROI 爆表, 配合 G1 一鍵 fork |
| 🥇 | **AN1** 跟 NPC 聊天 (Gemini chat) | 3 天 | 學生超愛 + Gemini 已串好, 加 chat UI |

### 🥈 一個月內（教學現場 + 老師生態）

| 優先 | 項目 | 工時 | 為什麼值得 |
|---|---|---|---|
| 🥈 | **AO2** 同校老師共筆 + 學年接班 | 4 天 | 學校教研社群形成的關鍵 |
| 🥈 | **AO3** 老師 Discord/LINE 社群 | 1 週籌備 | 形成教師同行社群, 收第一手反饋 |
| 🥈 | **AP3** 高對比 / 色盲友善模式 | 1.5 天 | a11y 完整化第二步 |
| 🥈 | **AN3** 給未來自己的一封信 | 1.5 天 | SEL 反思深化的浪漫設計 |
| 🥈 | **AN2** 7 天打卡練習系統 | 2 天 | SEL 從一次性變持續習慣 |
| 🥈 | **AM5** PWA 推播通知 | 2 天 | 老師活動提醒 + 學生打卡提醒 |

### 🥉 一學期內（內容深化 + 跨平台）

| 優先 | 項目 | 工時 | 為什麼重要 |
|---|---|---|---|
| 🥉 | **AM1** 國中初探版 Story Pack | 5-7 天 | 銜接國中, 擴大年齡層 |
| 🥉 | **AL3** 教學使用案例 podcast 5 集 | 2.5 天 | 教師同行 + 家長, 多管道擴散 |
| 🥉 | **AL5** 第一場線上教師工作坊 | 籌備 2 週 | 教師社群形成里程碑 |
| 🥉 | **AM2** 家長版 Story Pack | 5 天 | 親師活動 / 親子討論 |
| 🥉 | **AP5** 多語版本 (簡中 / 英 / 越南) | 7 天 | 新住民學生友善 + 國際推廣 |
| 🥉 | **AN4** AI 學習教練週報 | 2 天 | 學生持續成長伴侶 |

### 🌌 一年內 / 長期願景

| 項目 | 工時 | 為什麼夢想 |
|---|---|---|
| **AL2** 跟教育心理系合作學術研究 | 1 學期 | SSCI 期刊發表, 教育影響力 |
| **AL4** 教學白皮書中英雙語版 | 5 天 | 國際華僑學校推廣 |
| **AM3** LINE Bot 互動版 | 3 天 | 學生最熟悉平台 |
| **AM4** 桌遊 / 卡牌實體版 | 2 個月 | 沒網路的學校 / 偏鄉 / 安養院 |
| **AO5** 老師 AI 助手對話式備課 | 2 天 | 老師教案準備加速 |
| **AP4** 完整聲控操作 | 3 天 | 視障 / 行動不便 / 識字弱 |
| **AP6** 特教專用模式 | 4 天 | 自閉 / 學障 / 發展遲緩學生友善 |
...
### 🗓️ 第 1 週: a11y + 投稿準備 (個人專業影響力)
- 🥇 **AP1** axe-core 完整稽核 (2 天) → 學校採購硬需求
- 🥇 **AL1** 投稿教育月刊 1500-2000 字心得草稿 (3 天) → 為下個月投稿準備

### 🗓️ 第 2 週: 推廣 + 學生體驗深化
- 🥇 **AO1** fork 全國地圖 + 展示頁 (1.5 天) → 推廣 ROI 爆表
- 🥇 **AN1** 跟 NPC 聊天 Gemini chat (3 天) → 學生超愛

### 🗓️ 第 3 週: 老師生態 + 高對比
- 🥈 **AO3** Discord/LINE 社群籌備 (1 週) → 形成教師同行社群
- 🥈 **AP3** 高對比 / 色盲友善模式 (1.5 天)

### 🗓️ 第 4 週: SEL 持續習慣
- 🥈 **AN3** 給未來自己的一封信 (1.5 天)
- 🥈 **AN2** 7 天打卡練習系統 (2 天)

**完成 1 個月後**: 你會有「a11y 認證 + 投稿文章 + 全國地圖 + AI Chat + 教師社群 + 高對比 + SEL 持續習慣」整套, 可以發第二輪推廣 + 投教育月刊。

---

## 📊 4 大關鍵指標追蹤建議 (v3.22 後升級版)

| 指標 | 怎麼測 | 6 月目標 | 1 年目標 |
|---|---|---|---|
| **使用學校數** | AO1 全國地圖 + 主動回報表單 | ≥ 20 校 | ≥ 100 校 |
| **每月活躍老師** | OAuth 帳號 7 天內登入 dashboard | ≥ 100 位 | ≥ 500 位 |
| **學生完成三部曲數** | localStorage 跨次累積 | ≥ 1000 次 | ≥ 5000 次 |
| **Lighthouse 平均分** | AH4 CI 監測 | 維持 ≥ 95 | ≥ 95 (持續) |
| **教育月刊發表** | AL1 投稿 | 1 篇 | 3 篇 |
| **教師工作坊辦理** | AL5 月 1 場 | 3 場 (90 位老師) | 12 場 (600 位老師) |
| **學術論文** | AL2 跟教育系合作 | 開始合作 | 1 篇投稿 |

---

## 💡 整體策略建議

v3.0 → v3.22 走了 5 個月, **產品已經完整**, 接下來的重點要從「**做產品**」轉到「**做影響**」:

### 🎯 三個重點軸

1. **教育研究 / 學術合作** (AL 系列) — 把使用經驗轉化成可被引用的成果
2. **教師生態 / 跨校推廣** (AO 系列) — 從一個人做 → 教師社群一起做
3. **無障礙完整化** (AP 系列) — 學校採購硬需求 + 社會責任

### 🚀 策略性順序

「先有採購文件 (AP1 axe-core) → 再寫推廣文章 (AL1) → 再辦工作坊 (AL5) → 再做學術研究 (AL2)」
